...
Talk to a Home Assistant instance through this plugin's MCP tools. The tools wrap the REST API (`GET /api/`, `GET /api/states`, `GET /api/states/{entity_id}`, `POST /api/services/{domain}/{service}`). Do not use Home Assistant's native `/api/mcp` endpoint. That needs a separate HA integration this plugin does not install.
...
2. To find entities, call `ha_list_states` with a `domain` (`light`, `switch`, `sensor`) or an `entity_id` `prefix` (`light.kitchen`, `sensor.weather`). Unfiltered dumps are rejected. Results are capped (default 50, max 100). If `truncated` is true, narrow the prefix.
...
- Call `ha_list_states` without `domain` or `prefix`
